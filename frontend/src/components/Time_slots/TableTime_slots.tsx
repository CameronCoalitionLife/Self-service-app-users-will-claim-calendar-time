...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/time_slots/time_slotsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureTime_slotsCols';
...
const TableSampleTime_slots = ({
  ...
  const {
    ...
    loading,
    count,
    notify: time_slotsNotify,
    refetch,
  } = useAppSelector((state) => state.time_slots);
  ...
  useEffect(() => {
    if (time_slotsNotify.showNotification) {
      notify(
        time_slotsNotify.typeNotification,
        time_slotsNotify.textNotification,
      );
    }
  }, [time_slotsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/time_slots/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/time_slots/time_slots-view/?id=${id}`);
  };

  const handleCreateEventAction = ({ start, end }: SlotInfo) => {
    router.push(
      `/time_slots/time_slots-new?dateRangeStart=${start.toISOString()}&dateRangeEnd=${end.toISOString()}`,
    );
  };
...
export default TableSampleTime_slots;
