...
export default {
  ...
  time_slotsManyListFormatter(val) {
    if (!val || !val.length) return [];
    return val.map((item) => item.start_time);
  },
  time_slotsOneListFormatter(val) {
    if (!val) return '';
    return val.start_time;
  },
  time_slotsManyListFormatterEdit(val) {
    if (!val || !val.length) return [];
    return val.map((item) => {
      return { id: item.id, label: item.start_time };
    });
  },
  time_slotsOneListFormatterEdit(val) {
    if (!val) return '';
    return { label: val.start_time, id: val.id };
  },
